    ...
    With banner
        ...
        With .TextFrame2
            .TextRange.Text = "科研台账 (keyantaizhang) 课题组学术家底一账摸清" & vbCrLf & _
                              "全流程自动化 · 100% 动态逐行扫描 · SCI/EI/中文核心 · 双工作表 10 列直出 (含DOI超链接) (含影响因子)"
...
Public Sub 一键执行业务全流程()
    ...
    answer = MsgBox("确定要启动【一键自动化执行全流程】吗？" & vbCrLf & vbCrLf & _
                    ...
                    "  3. 智能消歧认领、跨库去重并直出 10 列交付大表 (含通讯作者/影响因子/DOI超链接) (含通讯作者与影响因子) (含影响因子)；" & vbCrLf & _
    ...
    ' 3. 全面刷新数据看板
    Call 刷新控制面板数据
    Call AppendLog("【全流程完成】已成功直出 10 列交付大表 (含通讯作者/影响因子/DOI超链接) (含通讯作者与影响因子) (含影响因子) papers_final_merged.xlsx！")
    On Error GoTo 0
    ...
    summaryReport = "【 科研台账 (keyantaizhang) 全流程交付报告 】" & vbCrLf & _
                    ...
                    "二、 课题组正式入库成果 (10 列标准终稿表 (含通讯作者/影响因子/DOI超链接) (含通讯作者与影响因子)，含影响因子)：" & vbCrLf & _
                    ...
                    "四、 成果交付文件：" & vbCrLf & _
                    "  - 最终成果大表：papers_final_merged.xlsx (10 大核心字段 (含可点击 DOI)已就绪，含影响因子)"
